...
Tensor FFModel::reshape(Tensor const input,
...
Reshape::Reshape(FFModel &model,
  ...
  for (int i = 0; i < input->num_dims; i++) {
    ...
    //           << ", parallelidx: " << input->dims[i].parallel_idx << ".
    //           degree: " << input->dims[i].degree
  ...
  for (int i = 0; i < numdim; i++) {
    if (expanded && i < numdim - 1 &&
      ...
      // inherit
      dims[numdim - i - 1].size = _shape[i];
      dims[numdim - i - 1].degree = input->dims[input->num_dims - 2 - i].degree;
      dims[numdim - i - 1].parallel_idx =
