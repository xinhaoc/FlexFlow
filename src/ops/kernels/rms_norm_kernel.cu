...
__global__ void rms_forward(float const *input,
                               float const *weight,
                               float *output,
                               coord_t volume) {
...
void forward_kernel(float const *input_ptr,
  ...
  rms_forward<IndexType>
      <<<GET_BLOCKS(volume), CUDA_NUM_THREADS, 0, stream>>>(
          input_ptr, weight_ptr, output_ptr, volume);
}

__global__ void rms_backward(float const *output_grad,
